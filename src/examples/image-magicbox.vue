<template lang="html">
  <div class="examples-item">
    <div class="image-magicbox-examples">
      <peony-image-magicbox
        :list="list"
        v-model="index"
    ...
    <div>
      ...
      <select v-model="imageMode">
        ...
        <option value="both">图片模式：全背景+多功能</option>
        <option value="bg">图片模式：全背景</option>
        <option value="multi">图片模式：多功能</option>
      </select>
      <button v-if="!showThumbsNav" @click="showThumbsNav = true">开启缩略图导航</button>
      <button v-else @click="showThumbsNav = false">关闭缩略图导航</button>
...
export default {
  data () {
    return {
      ...
      showMore: true,
      imageMode: 'both',
      carouselSpeed: 3000, // 幻灯片播放间隔时间-毫秒
      showThumbsNav: false, // 是否显示缩略图导航
      list: [
        {
          name: '0.jpg', // 图片名称
          url: image0 // 图片url路径
  ...
  methods: {
    handleThumbsUrlMethod (action, url) {
      return action + url
    },
